...
/**
 * Fetch containers with status 'available' or 'full', ordered by departure_date ASC.
 * Merges pending booking request count into each container.
 */
export async function fetchAvailableContainers(): Promise<ContainerWithPendingCount[] | null> {
  ...
  try {
    ...
    if (!containerResp.ok) {
      log({
        level: "error",
        msg: "Failed to fetch shared containers",
        route: "supabase-containers",
        status: containerResp.status,
        body: await containerResp.text(),
      });
      return null;
    }
    ...
    // Fetch pending request counts grouped by container_id
    const containerIds = containers.map((c) => c.id);
    const requestParams = new URLSearchParams({
      select: "container_id",
      status: "in.(new,contacted,quoted)",
      container_id: `in.(${containerIds.join(",")})`,
    });
    ...
    if (requestResp.ok) {
      const requests = (await requestResp.json()) as { container_id: string }[];
      for (const req of requests) {
        countMap.set(req.container_id, (countMap.get(req.container_id) ?? 0) + 1);
      }
    } else {
      log({
        level: "warn",
        msg: "Failed to fetch pending request counts, defaulting to 0",
        route: "supabase-containers",
    ...
    log({
      level: "error",
      msg: "fetchAvailableContainers error",
      route: "supabase-containers",
